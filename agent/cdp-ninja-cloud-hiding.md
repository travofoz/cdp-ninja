...
### Session Management
- **Parent session**: Main performance conversation
- **Child sessions**: Deep-dive performance analysis
- **Navigation**: Ctrl+Left/Right between performance specialties
- **Context preservation**: Performance context and optimization state

### Command Integration
- **@cdp-ninja-cloud-hiding** → Direct performance debugging
- **/debug-cdp** → Hidden Door routes here for performance issues
- **Tab switching** → Move to Build agent for performance fixes
- **Session history** → Maintain performance investigation context

The Cloud Hiding school brings unparalleled performance observation expertise to the CDP Ninja system, silently monitoring the digital atmosphere and optimizing the invisible forces that shape user experience with the stealth and precision of a master ninja hidden among the clouds.